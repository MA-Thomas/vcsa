...
fcn = 'feval';
data = 'saxs_Diff_Eq_MT';	% select test function from Jones et al. test set
		% bra  n=2   Branin
...
% u(k)<v(k) is required
%
% % %MT [u,v,fglob] = defaults(data); 	% returns bounds used by Jones et al.
				% and known global optimum
dimension=12;%length(u)		% show dimension
fglob = 0; %MT
...
if use_defaults, 
  ...
  % This sets u and v, amoung other needed variables.
  pre_runMCS_DiffEqs_Assembly
  tic;
  ...
  runtime = toc
  save('MCS_search_u_v_1_200_run1.mat')
  return;
end;
