...
%         covfunc = @covRQard; hyp2.cov = zeros(Dim+2,1); hyp2.lik = log(0.1)
        R = Dim;
        
        if cov_iter == 1
        covfunc = {@covMaternard,3};hyp2.cov = zeros(Dim+1,1);hyp2.lik = log(0.1);
        elseif cov_iter == 2
        covfunc = {@covMaternard,5};hyp2.cov = zeros(Dim+1,1);hyp2.lik = log(0.1);
        elseif cov_iter == 3
        covfunc = @covRQard; hyp2.cov = zeros(Dim+2,1); hyp2.lik = log(0.1);
        elseif cov_iter == 4
        covfunc = @covRQiso; hyp2.cov = [0;0;0]; hyp2.lik = log(0.1);
        elseif cov_iter == 5
        covfunc = @covGaborard; hyp2.cov = zeros(Dim+Dim,1); hyp2.lik = log(0.1);
        elseif cov_iter == 6
        covfunc = @covNNone; hyp2.cov = [0; 0]; hyp2.lik = log(0.1);
        elseif cov_iter == 7
        covfunc = @covSEard; hyp2.cov = zeros(Dim+1,1); hyp2.lik = log(0.1);
